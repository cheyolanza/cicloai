...
class BikeRaceCategory(Base):
    ...
    __table_args__ = (UniqueConstraint("race_id", "category_id", name="uq_bike_race_categories_pair"),)
    ...
    race_id: Mapped[PythonUUID] = mapped_column(
        PostgresUUID(as_uuid=True), ForeignKey("bike_races.id", ondelete="CASCADE"), nullable=False, index=True
    )
    category_id: Mapped[PythonUUID] = mapped_column(
        PostgresUUID(as_uuid=True), ForeignKey("categories.id", ondelete="CASCADE"), nullable=False, index=True
    )
