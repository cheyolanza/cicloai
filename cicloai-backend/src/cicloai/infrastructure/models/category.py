...
class Category(Base):
    ...
    __table_args__ = (UniqueConstraint("name", "sex", "category_type", name="uq_categories_name_sex_type"),)

    id: Mapped[PythonUUID] = mapped_column(PostgresUUID(as_uuid=True), primary_key=True, default=uuid4)
    name: Mapped[str] = mapped_column(String(100), nullable=False, index=True)
    category_type: Mapped[str] = mapped_column(String(20), nullable=False, default="Federado", index=True)
