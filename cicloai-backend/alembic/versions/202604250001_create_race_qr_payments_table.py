...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["bike_race_id"], ["bike_races.id"], ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(
            ["competition_biker_id"], ["competition_bikers.id"], ondelete="SET NULL"
        ),
        sa.UniqueConstraint(
            "id_transaction", name="uq_race_qr_payments_id_transaction"
        ),
    )
    op.create_index(
        "ix_race_qr_payments_bike_race_id", "race_qr_payments", ["bike_race_id"]
    )
    op.create_index(
        "ix_race_qr_payments_competition_biker_id",
        "race_qr_payments",
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_race_qr_payments_competition_biker_id", table_name="race_qr_payments"
    )
